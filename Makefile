...
install:
	@echo "=== Installing ADV RAG dependencies ==="
	@which uv > /dev/null 2>&1 || (echo "Error: uv not found. Install from https://astral.sh/uv" && exit 1)
	@python3 --version | grep -q "Python 3.12" || (echo "Warning: Python 3.12 recommended, got $(python3 --version)" )
